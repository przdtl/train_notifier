...
async def run_command_handler(message: Message) -> None:
    await message.answer('Получаем данные о поездах из Новосибирска в Иркутск на 31.12.2024...')
    ...
    coros = [get_tickets_info(train.get('url'), index) for index, train in enumerate(trains, 1) if train.get('url') is not None]

    result = await asyncio.gather(*coros)

    for train_tickets in result:
        for ticket in train_tickets:
            message.answer(f'tickets count for car №{ticket.get('carriage_number')} = {len(train_tickets)}')
