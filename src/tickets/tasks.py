...
@broker.task
async def get_tickets_recent_info(
    train_id: int, train_url: str, ticket_service: RailwayTicketServices
) -> list[Ticket]:
    parser = parser_factory.create_parser(ticket_service)
    tickets = await parser.parse(train_url)

    synchronize_tickets_list = await synchronize_tickets_info(train_id, tickets)

    return synchronize_tickets_list
